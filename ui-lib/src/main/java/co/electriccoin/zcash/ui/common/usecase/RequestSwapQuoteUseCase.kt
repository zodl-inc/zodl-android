...
class RequestSwapQuoteUseCase(
    ...
    suspend fun requestExactInput(
        ...
        val newAddress = accountDataSource.getSelectedAccount().transparentAddress
        requestQuote(
            requestQuote = {
    ...
    /**
     * Maya execution is gated on SDK OP_RETURN support (MOB-1396 phase 1), so no ZEC deposit proposal is built
     * for a Maya-selected quote: the quote is still shown, and confirm stays disabled for Maya until phase 2.
     * See `docs/SwapKit Spec (Maya DEX).md`.
     */
    @Suppress("TooGenericExceptionCaught")

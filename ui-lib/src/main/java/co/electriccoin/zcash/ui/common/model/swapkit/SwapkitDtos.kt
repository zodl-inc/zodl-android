...
/** Response of `GET /tokens?provider=MAYACHAIN`. */
...
/** Request body of `POST /price`; the response is a JSON array of [SwapkitPriceDto]. */
@JsonIgnoreUnknownKeys
@Serializable
...
data class SwapkitQuoteRequestDto(
    ...
    /**
     * Binds the route to real addresses at quote time: `/v3/swap` rejects a route built without them with
...
data class SwapkitSwapRequestDto(
    ...
    /** Zodl skips the PSBT/balance work it cannot do for shielded Zcash; only `targetAddress` + `memo` are used. */
...
data class SwapkitSwapResponseDto(
    ...
    /**
     * Epoch "valid until": Maya will not accept a deposit broadcast after this (~60s + leeway). Used as the
     * swap deadline.
     */

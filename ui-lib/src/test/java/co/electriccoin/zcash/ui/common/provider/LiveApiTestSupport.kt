...
/** Hardcoded Zcash unified test address, broken across lines only to satisfy the line-length limit. */
...
/**
 * Gate for the live swap-API integration tests ([KtorNearApiProviderLiveTest], [KtorSwapkitApiProviderLiveTest]).
 * They hit the real NEAR 1Click and SwapKit/Maya APIs over the network, so they are skipped unless
 * `ZCASH_RUN_LIVE_SWAP_API_TESTS=true` is set and must never run in CI. Run them manually with, e.g.:
 *
 * ```
...
/**
 * A real network-backed [HttpClientProvider] (clearnet, no Tor) for the live integration tests. Logs full
 * requests and responses, bodies included, so live runs show exactly what hits the wire.
 */

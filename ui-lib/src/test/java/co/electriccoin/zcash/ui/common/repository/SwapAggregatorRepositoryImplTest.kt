...
class SwapAggregatorRepositoryImplTest {
    ...
    /** Maya answers first while NEAR is still in flight; the aggregate resolves only once NEAR also settles. */
    @Test
    ...
    /**
     * A stale Maya Success from a previous round must be cleared, not left in place, when the requested asset is
     * a plain NEAR-only [co.electriccoin.zcash.ui.common.model.near.NearSwapAsset]; and a permanently null
     * (non-participating) Maya must not block settlement once NEAR resolves.
     */
    @Test
